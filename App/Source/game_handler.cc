...
bool GameHandler::validateTurn(Piles playedCards) {
  ...
  if (!this->inCorrectOrder(this->piles.firstone(), playedCards.firstone(),
                            true)) {
    return false;
  }
  if (!this->inCorrectOrder(this->piles.secondone(), playedCards.secondone(),
                            true)) {
    return false;
  }
  if (!this->inCorrectOrder(this->piles.firsthundred(),
                            playedCards.firsthundred(), false)) {
    return false;
  }
  if (!this->inCorrectOrder(this->piles.secondhundred(),
                            playedCards.secondhundred(), false)) {
    return false;
  }
  return true;
}
...
bool GameHandler::playedCorrectlyOnPile(Card card, Pile pile, bool increasing) {
  ...
  for (Card card : playedCards.cards()) {
    if (increasing) {
      if (!(card.value() > previousCardValue ||
            card.value() - 10 == previousCardValue)) {
        return false;
      }
    } else {
      if (!(card.value() < previousCardValue ||
